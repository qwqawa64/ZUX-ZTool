这个项目更适合做“UI 层重构”，不建议在现有 XML/Fragment/Activity 结构上逐页硬迁
  ...
  - `settingactivity/setting/floatingwindow/FloatingWindow.kt`：一视界适配悬浮窗已由 XML View 迁移为 `WindowManager` 承载 `ComposeView`，保留 `SettingsDetailActivity` 构造/隐藏调用入口；保留步骤向导、当前前台应用/Activity 轮询、添加活动、配置选项、教程视频循环播放、Base64 配置保存和关闭/隐藏逻辑。
  ...
  - 下一步可继续评估 `utils/AppChooserDialog.java`。该弹窗被游戏助手、桌面和系统设置详情页复用，应保留现有 `AppSelectionCallback` 调用契约。
  ...
  - 已完成并验证：`FloatingWindow` Compose 化。
  - 下一次继续时，建议评估 `AppChooserDialog.java`，将复用应用选择弹窗迁移为 Compose Dialog，同时保留现有回调和调用点。
  ...
  2. `settingactivity/setting/floatingwindow/FloatingWindow.java`（已完成）
